...
export default function ValidarValeModal(
  ...
  // Helper para mostrar timestamp legible del vale (creación)
  const fechaCreacion = vale.fecha || ''
  const horaCreacion = (vale as any).hora || ''

  // Helper para mostrar desglose C/B/U en el detalle
  const formatDetalleProducto = (d: ValeDetalle) => {
    const total = d.totalUnidades
    const partes: string[] = []
    if (d.cajas) partes.push(`${d.cajas}C`)
    if (d.bandejas) partes.push(`${d.bandejas}B`)
    if (d.unidades) partes.push(`${d.unidades}U`)
        ...
        <div className="p-6 bg-gradient-to-r from-blue-600 to-blue-800 text-white flex justify-between items-center rounded-t-2xl">
          <div>
            ...
            <p className="text-blue-100 mt-1">
              Confirma si deseas validar o rechazar el vale de ingreso.
            </p>
          </div>
          <button
            onClick={onClose}
