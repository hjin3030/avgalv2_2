...
// Normaliza ids para que "sala L", "SalaL", "salal" se vuelvan comparables
...
export default function Sidebar() {
  ...
  const allowedModulesSet = new Set(
    (profile?.modulosPermitidos || []).map((m) => normalizeModuleId(m))
  )
  ...
  const availableModules = modules.filter((mod) => {
    ...
    const modKey = normalizeModuleId(mod.module)
    return allowedModulesSet.has(modKey)
  })
  ...
  return (
    <>
      ...
      <aside
        className={[
          'fixed top-0 left-0 z-50 h-screen w-64 bg-white border-r border-gray-200 transition-transform',
          'lg:translate-x-0',
